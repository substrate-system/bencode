...
test('should be throw an error when trying to decode a broken float', (t) => {
    t.plan(2)
    t.throws(function () {
...
test('should be able to decode a string', function (t) {
    ...
    t.deepEqual(
        bencode.decode(data.binResultData.toString()),
        new Uint8Array(data.binStringData)
    )
})
...
test('should be able to decode stuff in dicts (issue #12)', function (t) {
    ...
    t.equal((dat as Record<string, any>).integer, 12345)
    t.deepEqual(
        (dat as Record<string, any>).string,
        new Uint8Array(Buffer.from('Hello World'))
    )
    t.deepEqual(
        (dat as Record<string, any>).dict.key,
        new Uint8Array(Buffer.from('This is a string within a dictionary'))
    )
    t.deepEqual(
        (dat as Record<string, any>).list,
        [1, 2, 3, 4, new Uint8Array(Buffer.from('string')), 5, {}]
    )
})

// test('bencode#decode(x)', function (t) {
//     // these tests weren't actually correctly testing values, just mangling
//     //   values and checking if they are mangled, TODO: fix
